...
def chat_interaction(session):
    if session:
        ...
        if choice == "1":
            ...
            # Looping for continuing or dumping the chat for the new session
            while True:
                options = ["1. Continue the conversation", "2. Dump the old chat and start a new session"]
                print("\n".join(options))
                choice = input("Enter the number corresponding to your choice: ")
                if choice == "1":
                    ...
                    session.clear_chat_history()
                    print("Starting a new session.")
                    break  # Exit the loop and start a new session
...
session_obj = start_session()  # Store the returned session object
if session_obj:
    chat_interaction(session_obj)  # Pass the session object to chat_interaction